...
func GetServerIP() {
	...
	if l_config.AppIP != "" {
		...
		if err := pingOnce(l_config.AppIP); err == nil {
			logf("Using cached appIP (verified): %s", l_config.AppIP)
		} else {
			logf("Cached appIP %s is not reachable. Re-discovering... That may take a while. Please wait.", l_config.AppIP)
			l_config.AppIP = ""
		}
	}
	...
	if l_config.AppIP == "" {
		...
		l_config.AppIP = findIPByMAC(arpTable, l_config.MAC)
		if l_config.AppIP == "" {
			logf(true, "Could not find IP for MAC: %s", l_config.MAC)
		} else {
			err = SaveConfig()
			logf(err, "Failed to save config: %s", err)
			logf("Discovered appIP: %s for MAC: %s", l_config.AppIP, l_config.MAC)
		}
	}
}
