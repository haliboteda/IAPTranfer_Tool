...
#
# Copy to machine.py and edit. machine.py is gitignored; this template is not,
# so a new machine only ever has to fill in these values.
#
# tools/common.py imports it:  from common import cfg
#
...
# output directory, the CubeIDE plugin suffix -- tools/common.py derives all of
...
# are not on; keeping both means the second assignment silently wins.
...
A15 = os.environ.get("LOCALAPPDATA", "") + r"\Arduino15"
CORE_LIVE = A15 + r"\packages\OpenPLC_Alpha\hardware\stm32\0.1.3-pre"
# A15 = HOME + "/.arduino15"
...
# at the install root; common.py resolves the versioned plugin and the
# per-platform executable names underneath it.
CUBEIDE = r"D:\ST\STM32CubeIDE_1.10.0"
WORKSPACE = r"E:\WorkSpace\Schaeffer-AG"            # Eclipse workspace holding the bootloader project
# CUBEIDE = "/opt/st/stm32cubeide_1.10.0"
# WORKSPACE = HOME + "/work/schaeffer"

# IAPTool ships inside the board package. Leave this empty and common.py finds it
# by wildcard under A15 for the current platform -- which is what you want,
...
ARDUINO_CLI = IDE + r"\resources\app\lib\backend\resources\arduino-cli.exe"
ARDUINO_CLI_CONFIG = HOME + r"\.arduinoIDE\arduino-cli.yaml"
# IDE = "/opt/arduino-ide"
...
#
# It must be a MODERN gcc/clang. A Dev-C++ install ships GCC 3.4.2 (2004): it
# rejects -std=c11 outright, and at -std=c99 its linker crashes. Verifying code
# destined for arm-none-eabi-gcc 12.x with a twenty-year-old compiler would be
