...
Four groups, in the order the file is used:
  ...
  tables       -- SETTINGS, PREREQS and EXAMPLES are hand-edited lists, and the
...
def test_tables():
    ...
    for name, _what, checker, _req, _inst in im.PREREQS:
        ...
        check("%s reports a bool and a reason" % name, good, True)
...
def test_repo_branch(tmp):
    """A fresh clone lands on the remote's default branch, which on this product
    is not where the work is. This machine cannot show it -- its clones predate
    origin/HEAD being set -- so the ref is written by hand here."""
    print("=== repo_branch ===")
    repo = tmp / "branchy"
    repo.mkdir()
    git(repo, "init", "-q")
    ...
    git(repo, "checkout", "-q", "-b", "v0.1.3.1")

    current, default = im.repo_branch(repo)
    check("the checked-out branch is reported", current, "v0.1.3.1")
    check("no origin/HEAD means the default is unknown, not guessed", default, "")
    ...
    current, default = im.repo_branch(repo)
    check("the remote default is read from the local ref, offline", default, "master")
    check("and it differs from the working branch, which is the point",
